...
set.seed(42)
percent <- 10
subsamp <- sample(nrow(tumor.hc.rank), (1 / percent) * nrow(tumor.hc.rank))
```
...
tumor.hc.umap <- umap(tumor.hc.norm[subsamp, ], n_threads = 5)
```
...
```{r}
cache <- paste0("output/nmf.rank.", percent, ".rds")
...
if (file.exists(cache)) {
  ...
  write_rds(tumor.hc.nmf.rank, cache)
}
...
```{r}
if(percent <= 5) coefmap(tumor.hc.nmf)
```
