...
We use an efficient implementation of alternating non negative least-squares with regularized to favor sparse basis matrices snmf/l.

```{r}
tumor.hc.nmf.rank <- nmfEstimateRank(as.matrix(t(tumor.hc.rank[subsamp,])), 
                                     range = seq(2,6), method = "snmf/l",
                                     nrun = 10, seed = 42)

plot(tumor.hc.nmf.rank)

tumor.hc.nmf <- nmf(as.matrix(t(tumor.hc.rank[subsamp,])), 5,
                method = "snmf/l", nrun = 10, seed = 42)
...
tumor.hc.clustered.nmf <- tumor.hc.winsorized[subsamp, ] %>%
  mutate(Cluster = as.factor(nmf.clusters)) %>%
  pivot_longer(names_to = "Marker", values_to = "Value", -Cluster)

profiles <- seq_len(max(nmf.clusters)) %>% map(~
ggplot(
  tumor.hc.clustered.nmf %>% filter(Cluster == .x),
  aes(x = Marker, y = Value, color = Marker)
) +
...
```{r}
silhouette.nmf <- silhouette(nmf.clusters, dist(tumor.hc.winsorized[subsamp,]))
...
tumor.hc.core.samples <- tumor.hc.winsorized[subsamp,] %>% add_column(Cluster = nmf.clusters) %>% 
  slice(core.samples)
```
