...
if (file.exists(cache)) {
  ...
  tumor.hc.nmf <- nmfEstimateRank(as.matrix(t(tumor.hc.rank[subsamp, ])),
...
samples  <- sample(unique(spatial$sample), 3)

samples %>% walk(\(s){
  output.fig <- paste0("output/cores_3/", s, ".png")
  if (!file.exists(output.fig)) {
    png(output.fig, width = 800, height = 800)
    (ggplot(spatial %>% filter(sample == s), aes(x = X, y = Y, color = Cluster)) +
      geom_point() +
      theme_map()) %>%
      print()
    dev.off()
